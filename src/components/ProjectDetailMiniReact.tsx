...
interface ProjectDetailMiniReactProps {
  /** 메인 페이지로 돌아가는 콜백 함수 */
  onBack: () => void;
}

export function ProjectDetailMiniReact({ onBack }: ProjectDetailMiniReactProps) {
      ...
      <section className="py-24 px-6 bg-gradient-to-br from-green-50 to-teal-50">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-lg text-[#202632] leading-relaxed max-w-3xl">
            프레임워크의 본질적인 로직에 대한 깊은 이해를 위해, VDOM, Hook, 재조정 엔진을 Vanilla JS로 직접 설계하고 구현했습니다.
          </p>
        </div>
      </section>
        ...
        >
          ...
          <div className="bg-[#F9FAFB] p-8 rounded-2xl mb-12">
            <p className="text-[#202632] leading-relaxed text-lg mb-4">
              React를 사용하면서 VDOM, Hook 등 핵심 동작 원리에 대한 깊은 이해 없이 '사용'만 하고 있다는 의문을 느꼈습니다. 
              React의 본질을 이해하기 위해 VDOM, Hook, 재조정 엔진(Reconciliation)을 Vanilla JS로 직접 설계하고 구현하는 것을 목표로 삼았습니다.
            </p>
            <p className="text-[#202632] leading-relaxed text-lg mb-4">
              <strong>(Build)</strong> Esbuild의 <code className="bg-white px-2 py-1 rounded text-sm">jsxFactory</code>를 오버라이드하여 JSX가 커스텀 VNode 객체로 변환되는 과정을 통제했습니다. 
              <strong>(Render)</strong> 'Symbol'과 'WeakSet'을 이용한 효율적인 이벤트 위임 아키텍처를 구현했습니다. 
              <strong>(Reconcile)</strong> 'key' 기반의 지능형 Diffing 알고리즘(<code className="bg-white px-2 py-1 rounded text-sm">diffTargetFinder</code>)을 설계하여 DOM 업데이트를 최소화했습니다.
            </p>
            <p className="text-[#202632] leading-relaxed text-lg">
              빌드-렌더링-재조정으로 이어지는 <strong>React의 핵심 성능 최적화 원리를 증명하고 내재화</strong>했으며, 
              JSX가 VNode 객체로 변환되는 과정을 완벽히 통제할 수 있게 되었습니다.
            </p>
          </div>
          ...
          <div className="space-y-12">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-8 rounded-2xl">
              <div className="flex items-center gap-3 mb-6">
                <div className="bg-blue-500 text-white px-4 py-2 rounded-lg" style={{ fontWeight: 700 }}>
                  1
                </div>
              ...
              <div className="mb-6">
                <div className="text-[1.125rem] text-[#0064FF] mb-4" style={{ fontWeight: 600 }}>
                  Esbuild jsxFactory 오버라이드
                </div>
                <p className="text-[#202632] leading-relaxed">
                  Vite의 Esbuild를 단순 도구로 사용하는 것을 넘어, <code className="bg-white px-2 py-1 rounded text-sm">jsxFactory</code> 옵션을 자체 구현한 'createElement' 함수로 연결했습니다. 이를 통해 JSX 구문이 VNode 객체로 변환되는 과정을 완벽하게 통제하고, 빌드 설정과 프레임워크 코어가 연결되는 과정을 내재화했습니다.
                </p>
              </div>

              {/* Code Block - vite.config.ts */}
              <div className="bg-[#202632] p-6 rounded-xl overflow-x-auto">
                <div className="text-sm text-gray-400 mb-2">vite.config.ts</div>
    ...
    esbuildOptions: {
      ...
      jsxFragment: 'Fragment',
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-8 rounded-2xl">
              <div className="flex items-center gap-3 mb-6">
                <div className="bg-purple-500 text-white px-4 py-2 rounded-lg" style={{ fontWeight: 700 }}>
                  2
                </div>
                <h3 className="text-[1.5rem]" style={{ fontWeight: 600 }}>
                  렌더링 파이프라인과 이벤트 위임
              ...
              <div className="mb-6">
                ...
                <p className="text-[#202632] leading-relaxed mb-4">
                  <code className="bg-white px-2 py-1 rounded text-sm">internalRender</code> 함수를 설계하여, 함수형 컴포넌트(Function VNode)와 네이티브 태그(String VNode)를 구분하는 재귀적 렌더링 파이프라인을 구축했습니다.
                </p>
                <p className="text-[#202632] leading-relaxed">
                  또한, 루트 컨테이너에 <code className="bg-white px-2 py-1 rounded text-sm">WeakSet</code>으로 단 한 번만 리스너를 등록하고, <code className="bg-white px-2 py-1 rounded text-sm">Symbol</code> 키를 사용해 DOM 오염 없이 핸들러를 저장하는 효율적인 이벤트 위임 아키텍처를 구현했습니다.
                </p>
              </div>
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-cyan-50 p-8 rounded-2xl">
              <div className="flex items-center gap-3 mb-6">
                <div className="bg-emerald-500 text-white px-4 py-2 rounded-lg" style={{ fontWeight: 700 }}>
                  3
                </div>
                <h3 className="text-[1.5rem]" style={{ fontWeight: 600 }}>
              ...
              <div className="mb-6">
                ...
                <p className="text-[#202632] leading-relaxed mb-4">
                  React 성능의 핵심인 'Key' 기반의 지능형 Diffing 알고리즘을 설계했습니다. <code className="bg-white px-2 py-1 rounded text-sm">diffTargetFinder</code>를 구현하여, 'key'가 존재하면 'key'로, 없으면 'index'로 비교 대상을 찾는 지능형 탐색 로직을 완성했습니다.
                </p>
                <p className="text-[#202632] leading-relaxed">
                  이를 통해 불필요한 DOM 재생성을 방지하고 성능 최적화 원리를 증명했습니다.
                </p>
              </div>

              {/* Code Block - diffTargetFinder.ts */}
              <div className="bg-[#202632] p-6 rounded-xl overflow-x-auto">
                <div className="text-sm text-gray-400 mb-2">diffTargetFinder.ts</div>
                <pre className="text-sm text-white">
  ...
  if (target) {
    ...
    createElement(newVNode, parentElement);
      ...
      >
        ...
        >
          <h2 className="text-[2rem] mb-8 text-center" style={{ fontWeight: 700 }}>
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="flex-1">
                <p className="text-lg text-[#202632] leading-relaxed mb-6">
                  선언적 상태 변경(<code className="bg-[#F9FAFB] px-2 py-1 rounded text-sm">useState</code>)이 <code className="bg-[#F9FAFB] px-2 py-1 rounded text-sm">diff.ts</code>를 통해 변경된 부분만 선별적으로 DOM에 업데이트하는 Virtual DOM의 핵심 로직을 직접 구현했습니다.
                </p>
                <p className="text-lg text-[#202632] leading-relaxed">
                  Key 속성을 활용한 노드 탐색 및 재사용 로직을 구현함으로써, 프레임워크가 불필요한 DOM 조작을 최소화하고 리스트 업데이트 성능을 효율적으로 관리할 수 있는 아키텍처적 기반을 마련했습니다.
                </p>
